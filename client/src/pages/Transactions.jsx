...
import {
  fmtUSD, fmtIQD, fmtDate, fmtRelativeDate,
  iqdToUsdCents, todayISO,
} from '../lib/format.js';
...
export default function Transactions() {
  ...
  const [editing, setEditing] = useState(null); // tx object or null
      ...
      <div className="space-y-6">
        ...
        <div className="cw-card p-4 flex items-center gap-4 flex-wrap">
          ...
          {(filterCat || filterUser) && (
            <button
              onClick={() => { setFilterCat(''); setFilterUser(''); }}
              className="cw-btn-ghost text-sm self-end"
            >
              Clear filters
            </button>
          )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && txns.length === 0 && (
                <tr><td colSpan="8" className="text-center text-inkDim py-10">
                  No allocations yet. Log your first one.
                </td></tr>
              )}
              {txns.map((t) => (
...
function TxFormModal({ editing, onClose, onSaved }) {
  ...
  const [currency, setCurrency] = useState(editing?.amount_iqd != null ? 'USD' : 'USD');
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (currency === 'USD') {
        ...
        if (!Number.isFinite(v) || v <= 0) throw new Error('Enter a positive USD amount.');
        body.amount_usd_cents = Math.round(v * 100);
      } else {
        const v = parseFloat(iqd);
        if (!Number.isFinite(v) || v <= 0) throw new Error('Enter a positive IQD amount.');
        body.amount_iqd = v;
      }
  ...
  return (
    <div className="fixed inset-0 bg-bg/90 flex items-center justify-center px-4 z-50"
         onClick={onClose}>
